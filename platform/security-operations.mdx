...
LibOps turns common security responsibilities into platform defaults. We ensure that the secure path is the standard path for every institution.

<CardGroup cols={3}>
  <Card title="Fully Encrypted" icon="lock">
  ...
  <Card title="Automated Hardening" icon="shield-check">
...
For accounts with a GitHub username, LibOps also syncs repository collaborator permissions from membership. Owner/developer access grants write permission on affected site repositories, while viewer access grants read permission. **LibOps Task Agent** PR comment follow-up uses GitHub repository write permission as its authorization check.

## OAuth and Slack Integration

OAuth-based authentication and Slack integration help connect LibOps to the way teams already work. Platform identity, team communication, and operational workflows should not live in separate worlds.

Slack integration is especially useful for support, notifications, and team workflows around site activity.

## Tenant Isolation

Sites run on their own VMs. This single-tenant model gives each workload a dedicated compute boundary and keeps app-level issues scoped to the site where they occur.

For organizations evaluating risk, this is a simpler model to reason about than shared hosting where unrelated applications compete inside the same runtime.
