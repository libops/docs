...
LibOps is built on Docker Compose. That choice shapes every other choice we make — what we build, what we don't build, what we ask operators to learn, and what we can hand back to institutions if they ever want to walk away from the managed service.
...
<Tip>
These properties matter more for GLAM institutions, colleges, and universities than they would for a hyperscale consumer app — but they're true at any scale. We just don't need to optimize for the hyperscale case.
</Tip>
...
The applications hosted by most LibOps customers — Islandora repositories, OJS journals, Drupal sites, Omeka exhibits, ArchivesSpace instances — rarely need more than modest scaling. Adopting Kubernetes for those workloads adds substantial operational overhead with limited return.

<Check>
We could have spent engineering resources building Kubernetes operators for each LibOps-supported application family. We deliberately didn't. The goal of the platform is to **let institutions adopt open-source software without being blocked by infrastructure complexity** — and choosing Kubernetes would have reintroduced that complexity at the platform layer.
</Check>

## One pattern, from laptop to production

Every LibOps template uses the same shape.
...
Behind those targets is a Compose project that runs identically in development and production. Variations are confined to the environment — different secrets, different domain names, different scale parameters. The *structure* doesn't change.
...
<CardGroup cols={2}>
  <Card title="Standardized deployment" icon="ship" href="https://github.com/libops/cloud-compose">
    [`cloud-compose`](https://github.com/libops/cloud-compose) is a Terraform module that takes any Docker Compose project and deploys it to a Google Cloud VM. Because Compose is the input contract, the module works across all supported application families without per-app customization.
  </Card>
  <Card title="Operator CLI" icon="terminal" href="https://sitectl.libops.io">
    [`sitectl`](https://github.com/libops/sitectl) gives operators kubectl-style contexts for switching between local and remote Compose projects — without needing a Kubernetes cluster to do it.
  </Card>
  ...
  <Card title="AI-assisted customization" icon="wand-magic-sparkles" href="/platform/coding-agent-workflow">
    The LibOps Task Agent edits Compose-based projects through normal git workflows. A consistent project shape makes the agent's edits predictable and reviewable.
  </Card>
</CardGroup>
