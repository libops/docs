...
<Info>
LibOps is a transparent platform, not a black box. You get a full API, the `sitectl` CLI, GitHub review, and Docker Compose templates, so you keep visibility and control over every stack you operate here.
</Info>
...
From there, `sitectl` is useful when you want saved local and remote contexts, plugin commands, service helpers, logs, port forwarding, and rollouts in one CLI. The LibOps platform adds managed hosting, firewall rules, secrets, previews, membership, and API-backed automation around the same repository shape.
