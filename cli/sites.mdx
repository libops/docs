...
sitectl create site \
  --project-id proj_xyz789 \
  --name "islandora-production" \
  --domain "collections.example.edu" \
  --description "Public Islandora repository"
...
sitectl edit site site_123abc \
  --name "collections-production" \
  --description "Public collections site"

# Update all fields
sitectl edit site site_123abc \
  --name "journal-production" \
  --domain "journal.example.edu" \
  --description "Public journal publishing site"
```
...
STAGING_SITE=$(sitectl create site \
  ...
  --name "collections-review" \
  --domain "collections-review.example.edu" \
  --description "Review environment for the collections site" \
  --format json)
...
For first-class LibOps application templates, deploy runs the repository rollout command on the site VM. That command is normally:

```bash
./scripts/rollout.sh
```

The rollout script is the same entrypoint used by `make rollout` during local development.
...
<Tip>
**Use descriptive names**: Include environment and purpose in site names, such as `collections-production` or `journal-review`.
</Tip>
...
1. **Naming convention**: Use consistent naming like `{application}-{environment}`
2. **One site per supported workload**: Each supported Docker Compose deployment should be a separate site
...
sitectl create site \
  ...
  --name "collections-production" \
  --domain "collections.example.edu" \
  --description "Production collections site"

# Staging
sitectl create site \
  --project-id $PROJECT_ID \
  --name "collections-staging" \
  --domain "collections-staging.example.edu" \
  --description "Staging collections site for testing"

# Development
sitectl create site \
  --project-id $PROJECT_ID \
  --name "collections-dev" \
  --domain "collections-dev.example.edu" \
  --description "Development collections site"
```

### Related Supported Sites
...
# Journal publishing site
sitectl create site \
  --project-id $PROJECT_ID \
  --name "journal-production" \
  --domain "journal.example.edu" \
  --description "OJS publishing site"

# Omeka exhibit site
sitectl create site \
  --project-id $PROJECT_ID \
  --name "exhibits-production" \
  --domain "exhibits.example.edu" \
  --description "Omeka public exhibits"

# ArchivesSpace review site
sitectl create site \
  --project-id $PROJECT_ID \
  --name "archives-review" \
  --domain "archives-review.example.edu" \
  --description "ArchivesSpace review environment"
```
