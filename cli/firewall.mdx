...
By default, LibOps sites are allowlisted: only IPs covered by firewall rules can access them. To publish a production site for the public internet, add an explicit public site rule. See [Getting a Site Ready to Publish](/platform/getting-site-ready-to-publish).
...
- **Any IPv4 address**: Use `0.0.0.0/0` for public production web traffic
- **IPv6**: Supported (e.g., `2001:db8::/32`)
- **Any IPv6 address**: Use `::/0` only when the production site should accept public IPv6 traffic
...
sitectl create firewall \
  ...
  --cidr "203.0.113.0/24" \
  --description "Institution office network"

# Allow VPN (project-wide)
sitectl create firewall \
  --project-id proj_xyz789 \
  --cidr "198.51.100.0/24" \
  --description "Staff VPN range"
...
# Publish a production site to all IPv4 visitors
sitectl create firewall \
  --site-id site_prod123 \
  --cidr "0.0.0.0/0" \
...
6. **VPN preferred for private sites**: Use VPN with firewall rules for development, staging, and admin-only environments
7. **Public production is explicit**: Only add `0.0.0.0/0` or `::/0` to production sites that are ready to publish
...
```bash
# Organization rule: Staff on the institution network can access
sitectl create firewall \
  --organization-id org_abc123 \
  --cidr "203.0.113.0/24" \
  --description "Institution-wide office access"
